...
@SpringView(name = Level7TestView.VIEW_NAME)
public class Level7TestView extends VerticalLayout implements View{
    public static final String VIEW_NAME = "level7TestView";
    public static final String CURRENT_LEVEL = "7";
    ...
    private Map<Question, String> answersMap7;
    ...
    private void addSubmitButtonListener() {
        submitButton.addClickListener(clickEvent -> {
            ...
            answersMap7.keySet().forEach(key -> {
                if(answersMap7.get(key).equals(key.getCorrect())) userScore++;
            });
    ...
    private void showResult(String result) {
        ...
        if (result.equals("success")){
            resultLayout.addComponent(new Label(" WOW!!!"));
            resultLayout.addComponent(new Label(" You managed to finish the game! Amazing, congratulations !!!"));
        } else {
            resultLayout.addComponent(new Label(" Too bad, you didn't manage to pass ! Keep learning !"));
        }
        resultLayout.addComponent(new Label());
    ...
    private void addLevel7Questions() {
        levelService.getLevel7Questions(5, user.getDomain()).forEach(
                level7Question -> {
                    Question question = new Question(
                    ...
                    String answer = "";
                    prepareQuestion(question, answer, 7);
                }
        );
    }
    ...
    private void prepareQuestion(Question question, String answer, int level) {
        ...
        if(level == 1) {
            ...
            answersMap7.put(question, answer);
            options.addValueChangeListener(valueChangeEvent -> {
                String newAnswer = options.getSelectedItem().get();
                String questionCaption = options.getCaption();
                answersMap7.keySet().forEach(key->{
                    if(key.getQuestion().equals(questionCaption)) {
                        answersMap7.put(key, newAnswer);
                    }
                });
            });
        }
